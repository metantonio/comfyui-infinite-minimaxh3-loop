...
# Templates captured from the source workflow before base sanitization.
# They are used only when --last-frame is explicitly requested.
_LAST_FRAME_NODE_TEMPLATE = None
_LAST_FRAME_IMAGE_NODE_TEMPLATE = None
...
def sanitize_workflow_base(workflow):
    """
    Remove every fixed H3 last-frame target from the base workflow.

    The JSON workflow may have been saved with node 15 -> last_frame -> 92
    -> 93. That state must NEVER survive into the runtime workflow unless
    the user explicitly supplies --last-frame.

    This function is intentionally called immediately after loading the JSON,
    before the workflow is inspected, printed, modified, or submitted.
    """
    global _LAST_FRAME_NODE_TEMPLATE
    global _LAST_FRAME_IMAGE_NODE_TEMPLATE

    wf = json.loads(json.dumps(workflow))

    # Preserve the original helper-node definitions only as in-memory
    # templates. They will never be sent to ComfyUI unless --last-frame
    # is explicitly requested.
    _LAST_FRAME_NODE_TEMPLATE = json.loads(
        json.dumps(wf.get(LAST_FRAME_NODE))
    ) if LAST_FRAME_NODE in wf else None

    _LAST_FRAME_IMAGE_NODE_TEMPLATE = json.loads(
        json.dumps(wf.get(LAST_FRAME_IMAGE_NODE))
    ) if LAST_FRAME_IMAGE_NODE in wf else None

    if PROMPT_NODE not in wf:
        raise RuntimeError(f"Node {PROMPT_NODE} not found.")

    inputs = wf[PROMPT_NODE].setdefault("inputs", {})

    # Remove the actual H3 conditioning input.
    inputs.pop("last_frame", None)

    # Remove the target-image helper nodes so they cannot be executed
    # independently or be accidentally reconnected by inherited workflow data.
    wf.pop(LAST_FRAME_NODE, None)
    wf.pop(LAST_FRAME_IMAGE_NODE, None)

    # Remove any stale links in other nodes that reference the deleted nodes.
    for node_id, node in wf.items():
        ...
        for input_name, value in list(node_inputs.items()):
            if (
                ...
                and str(value[0]) in {
                    LAST_FRAME_NODE,
                    LAST_FRAME_IMAGE_NODE,
                }
            ):
                node_inputs.pop(input_name, None)

    # Hard invariant: a clean base workflow cannot contain H3 last_frame.
    if "last_frame" in wf[PROMPT_NODE].get("inputs", {}):
        raise RuntimeError(
            "INTERNAL ERROR: base workflow sanitization failed; "
            "node 15 still contains last_frame."
        )

    if LAST_FRAME_NODE in wf or LAST_FRAME_IMAGE_NODE in wf:
        raise RuntimeError(
            "INTERNAL ERROR: base workflow sanitization failed; "
            "last-frame helper nodes still exist."
        )

    return wf


def validate_runtime_workflow(workflow, allow_last_frame=False):
    """
    Validate the exact workflow payload that is about to be submitted.

    If allow_last_frame is False, ANY last_frame input/helper node causes
    an exception. This is the final safety barrier immediately before /prompt.
    """
    h3_inputs = workflow[PROMPT_NODE].get("inputs", {})

    has_last_frame = "last_frame" in h3_inputs
    has_target_nodes = (
        LAST_FRAME_NODE in workflow or
        LAST_FRAME_IMAGE_NODE in workflow
    )

    if not allow_last_frame:
        if has_last_frame or has_target_nodes:
            raise RuntimeError(
                "SAFETY ERROR: unauthorized last-frame target detected "
                "in the runtime payload. Refusing to submit to ComfyUI."
            )
    else:
        if not has_last_frame:
            raise RuntimeError(
                "SAFETY ERROR: --last-frame was requested but node 15 "
                "does not contain last_frame."
            )

        if LAST_FRAME_NODE not in workflow:
            raise RuntimeError(
                "SAFETY ERROR: --last-frame was requested but node 92 "
                "is missing."
            )

        if LAST_FRAME_IMAGE_NODE not in workflow:
            raise RuntimeError(
                "SAFETY ERROR: --last-frame was requested but node 93 "
                "is missing."
            )
...
def print_runtime_last_frame_state(workflow, allow_last_frame=False):
    """
    Print the effective runtime state, not the original JSON state.
    """
    h3_inputs = workflow[PROMPT_NODE].get("inputs", {})

    print(
        "RUNTIME LAST-FRAME POLICY:"
    )

    if allow_last_frame:
        print("  Explicit target: ENABLED")
        print(
            f"  node {PROMPT_NODE}.last_frame: "
            f"{h3_inputs.get('last_frame')}"
        )
        print(
            f"  node {LAST_FRAME_NODE}: "
            f"{'present' if LAST_FRAME_NODE in workflow else 'MISSING'}"
        )
        print(
            f"  node {LAST_FRAME_IMAGE_NODE}: "
            f"{'present' if LAST_FRAME_IMAGE_NODE in workflow else 'MISSING'}"
        )
    else:
        print("  Explicit target: DISABLED")
        print(
            f"  node {PROMPT_NODE}.last_frame: "
            f"{h3_inputs.get('last_frame', 'NONE')}"
        )
        print(
            f"  node {LAST_FRAME_NODE}: "
            f"{'present' if LAST_FRAME_NODE in workflow else 'removed'}"
        )
        print(
            f"  node {LAST_FRAME_IMAGE_NODE}: "
            f"{'present' if LAST_FRAME_IMAGE_NODE in workflow else 'removed'}"
        )
...
def modify_workflow(
    ...
    if uploaded_last_filename:
        ...
        # Recreate the helper nodes from the in-memory templates captured
        # before base sanitization. They are allowed only in this branch.
        if (
            _LAST_FRAME_NODE_TEMPLATE is None or
            _LAST_FRAME_IMAGE_NODE_TEMPLATE is None
        ):
            raise RuntimeError(
                "--last-frame was supplied, but the source workflow did not "
                "contain the required last-frame helper nodes."
            )

        wf[LAST_FRAME_NODE] = json.loads(
            json.dumps(_LAST_FRAME_NODE_TEMPLATE)
        )
        wf[LAST_FRAME_IMAGE_NODE] = json.loads(
            json.dumps(_LAST_FRAME_IMAGE_NODE_TEMPLATE)
        )
...
def _main_impl():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional ending target image for the starting loop. "
            "Without this flag, no H3 last_frame target is ever used."
        ),
    )
    ...
    # ---------------------------------------------------------------------
    # CRITICAL: sanitize the BASE workflow immediately.
    #
    # The JSON may still contain the old fixed last-frame target (92 -> 93).
    # Remove it before any check/print/loop can observe or submit it.
    # --last-frame is the ONLY mechanism allowed to reintroduce it.
    # ---------------------------------------------------------------------
    workflow = sanitize_workflow_base(workflow)

    print()
    print("BASE WORKFLOW SANITIZATION")
    print("  Fixed last-frame target: REMOVED")
    print(f"  node {LAST_FRAME_NODE}: removed")
    print(f"  node {LAST_FRAME_IMAGE_NODE}: removed")
    print("  node 15.last_frame: removed")
    print()
    ...
    for offset, prompt in enumerate(
        ...
        # FINAL PAYLOAD SAFETY CHECK immediately before submission.
        # No --last-frame means absolutely no last-frame target, on any loop.
        validate_runtime_workflow(
            wf,
            allow_last_frame=bool(comfy_last_image_name),
        )
        ...
        print_runtime_last_frame_state(
            wf,
            allow_last_frame=bool(comfy_last_image_name),
        )
